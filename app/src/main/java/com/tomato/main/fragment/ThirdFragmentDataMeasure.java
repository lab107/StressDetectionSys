 ...
 */
public class ThirdFragmentDataMeasure extends Fragment{
    private final int FRAGMENTCODE=3;     //Fragmmet的编号
    public ChartService mService = null;   //画图工具类,控制原始曲线的小表及弹出窗口的大表
    public ChartService mService1 = null;   //画图工具类,控制纵向梯度曲线的小表及弹出窗口的大表
    ...
    public ThirdFragmentDataMeasure(){}
...
//            int status = bundle.getInt("s");
//            double x = bundle.getDouble("x");
//            double[]y1 =bundle.getDoubleArray("y");
//            double[]y2 = bundle.getDoubleArray("y1");
//            mService.drawMeasureChart(x, y1,status );
//            mService1.drawMeasureChart(x,y2,status );
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.third_fragment_datameasure, container, false);
        //折线图
    ...
    public void initChartAndChannelButtonNum(int channelNum,float warningValue){
        //重新初始化画图工具类的参数
        mService = new ChartService(getActivity(), channelNum,2);
        mService.setXYMultipleSeriesDataset("");
        mService.setXYMultipleSeriesRenderer();
        mService.setWarningLine(warningValue);          //设置警戒线
        mService1 = new ChartService(getActivity(),channelNum-1,3);//channelNum-1
